...
CLI::App* add_noarg_stream(CLI::App& parent, const char* name, const char* desc, selected_cmd& sel)
{
    ...
    sub->callback([&sel] {
        sel.factory = [](binapi2::futures_usdm_api& c,
            ...
            co_return co_await exec_stream_with_recorder(c, Subscription{}, sink);
        };
    });
    return sub;
}
...
CLI::App* add_symbol_stream(CLI::App& parent, const char* name, const char* desc, selected_cmd& sel)
{
    ...
    sub->callback([&sel, opts] {
        sel.factory = [opts](binapi2::futures_usdm_api& c,
            ...
            co_return co_await exec_stream_with_recorder(c, std::move(s), sink);
        };
    });
    return sub;
}
...

    {
        ...
        sub->callback([&sel, opts] {
            sel.factory = [opts](binapi2::futures_usdm_api& c,
                ...
                s.symbol = opts->symbol;
                s.interval = lib::parse_enum<types::kline_interval_t>(opts->interval);
                co_return co_await exec_stream_with_recorder(c, std::move(s), sink);
            };
        });
    }
