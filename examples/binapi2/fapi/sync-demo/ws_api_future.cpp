...
//
// sync-demo: WS API with future — spawn, do other work, then future.get().
...
void ws_api_future(client& c)
{
    ...
    {
        std::cout << "=== WS API future: io_thread ===\n";
        binapi2::fapi::detail::io_thread io;
        auto future = boost::cobalt::spawn(io.context(), ws_book_ticker(c),
                                           boost::asio::use_future);
        std::cout << "  (doing other work while WS API request in flight...)\n";
        auto r = future.get();
        print_result(r);
    }
    ...
    {
        std::cout << "=== WS API future: std::async ===\n";
    ...
    {
        std::cout << "=== WS API future: manual io_context ===\n";
        boost::asio::io_context io;
        auto future = boost::cobalt::spawn(io, ws_book_ticker(c),
                                           boost::asio::use_future);
        io.run();
