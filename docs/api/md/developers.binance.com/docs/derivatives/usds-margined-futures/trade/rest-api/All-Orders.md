...
- **http_request**: GET `/fapi/v1/allOrders`
- **request_weight**: 5
...
- Notes:
- If `orderId` is set, it will get orders >= that `orderId`. Otherwise most recent orders are returned. The query time period must be less then 7 days( default as the recent 7 days).
