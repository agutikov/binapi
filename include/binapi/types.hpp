...
struct new_order_resp_type :
    boost::variant<new_order_info_ack_t, new_order_info_result_t, new_order_info_full_t, new_test_order_info_t>
{
    // ctor inheritance
    using boost::variant<new_order_info_ack_t, new_order_info_result_t, new_order_info_full_t, new_test_order_info_t>::variant;

    std::pair<e_trade_resp_type, const void*> get_response_type() const
    {
    ...
    {
        const auto r = get_response_type();
        assert(r.first == e_trade_resp_type::ACK || r.first == e_trade_resp_type::RESULT || r.first == e_trade_resp_type::FULL);
