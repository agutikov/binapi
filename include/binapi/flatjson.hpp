...
#define __FLATJSON__MAKE_ERROR_MESSAGE(msg) __FILE__ "(" __FLATJSON__STRINGIZE(__LINE__) "): " msg
...
inline const char*
fj_token_type_name(e_fj_token_type t)
{
    static const char* strs[] = { "INVALID", "STRING", "NUMBER", "BOOL", "NULL", "OBJECT", "OBJECT_END", "ARRAY", "ARRAY_END" };
...
inline const char*
fj_error_string(e_fj_error_code e)
{
    static const char* strs[] = {
        "OK", "INVALID", "INCOMPLETE", "NO_FREE_TOKENS", "KLEN_OVERFLOW", "VLEN_OVERFLOW", "CHILDS_OVERFLOW"
    };
...
#define _FJ_CASE_1(x) case x:

#define _FJ_CASE_3(S) _FJ_CASE_1(S) _FJ_CASE_1(S + 1) _FJ_CASE_1(S + 2)

#define _FJ_CASE_5(S) _FJ_CASE_3(S) _FJ_CASE_1(S + 3) _FJ_CASE_1(S + 4)

#define _FJ_CASE_10(S) _FJ_CASE_5(S) _FJ_CASE_5(S + 5)

#define _FJ_CASE_20(S) _FJ_CASE_10(S) _FJ_CASE_10(S + 10)

#define _FJ_CASE_40(S) _FJ_CASE_20(S) _FJ_CASE_20(S + 20)

template<typename CharT>
bool
fj_is_simple_type(CharT v)
{
...
template<typename CharT>
bool
fj_is_hex_digit4(CharT ch0, CharT ch1, CharT ch2, CharT ch3)
{
...
static typename std::enable_if<(std::is_integral<To>::value && std::is_unsigned<To>::value) && !std::is_same<To, bool>::value,
                               To>::type
conv_to(const char* ptr, std::size_t len)
{
    ...
    switch (len) {
        case 20:
            v = v + (str[len - 20] - '0') * 10000000000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 19:
            v = v + (str[len - 19] - '0') * 1000000000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 18:
            v = v + (str[len - 18] - '0') * 100000000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 17:
            v = v + (str[len - 17] - '0') * 10000000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 16:
            v = v + (str[len - 16] - '0') * 1000000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 15:
            v = v + (str[len - 15] - '0') * 100000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 14:
            v = v + (str[len - 14] - '0') * 10000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 13:
            v = v + (str[len - 13] - '0') * 1000000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 12:
            v = v + (str[len - 12] - '0') * 100000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 11:
            v = v + (str[len - 11] - '0') * 10000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 10:
            v = v + (str[len - 10] - '0') * 1000000000ull;
            __FLATJSON__FALLTHROUGH;
        case 9:
            v = v + (str[len - 9] - '0') * 100000000ull;
            __FLATJSON__FALLTHROUGH;
        case 8:
            v = v + (str[len - 8] - '0') * 10000000ull;
            __FLATJSON__FALLTHROUGH;
        case 7:
            v = v + (str[len - 7] - '0') * 1000000ull;
            __FLATJSON__FALLTHROUGH;
        case 6:
            v = v + (str[len - 6] - '0') * 100000ull;
            __FLATJSON__FALLTHROUGH;
        case 5:
            v = v + (str[len - 5] - '0') * 10000ull;
            __FLATJSON__FALLTHROUGH;
        case 4:
            v = v + (str[len - 4] - '0') * 1000ull;
            __FLATJSON__FALLTHROUGH;
        case 3:
            v = v + (str[len - 3] - '0') * 100ull;
            __FLATJSON__FALLTHROUGH;
        case 2:
            v = v + (str[len - 2] - '0') * 10ull;
            __FLATJSON__FALLTHROUGH;
        case 1:
            v = v + (str[len - 1] - '0') * 1ull;
            __FLATJSON__FALLTHROUGH;
        default:
            break;
    }
...
template<typename To>
static typename std::enable_if<(std::is_integral<To>::value && std::is_signed<To>::value) && !std::is_same<To, bool>::value,
                               To>::type
conv_to(const char* ptr, std::size_t len)
{
    if (*ptr == '-') {
...
template<typename Iterator>
inline void
fj_skip_ws(fj_parser<Iterator>* p)
{
    for (; p->js_cur < p->js_end && (*p->js_cur == ' ' || *p->js_cur == '\t' || *p->js_cur == '\r' || *p->js_cur == '\n');
         ++p->js_cur)
        ;
}

#define __FLATJSON__CUR_CHAR(p) ((fj_skip_ws(p)), (p->js_cur >= p->js_end ? ((char)-1) : *(p->js_cur)))

template<typename Iterator>
inline int
fj_check_and_skip(fj_parser<Iterator>* p, char expected)
{
...
template<typename Iterator>
inline int
fj_escape_len(Iterator s, std::ptrdiff_t len)
{
    switch (*s) {
        case 'u':
            return len < 6 ? FJ_EC_INCOMPLETE : fj_is_hex_digit4(*(s + 1), *(s + 2), *(s + 3), *(s + 4)) ? 5 : FJ_EC_INVALID;
        case '"':
        case '\\':
        case '/':
        case 'b':
...
inline int
fj_expect(fj_parser<Iterator>* p, const char (&s)[ExLen], Iterator* ptr, std::size_t* size)
{
...
int
fj_parse_string(fj_parser<Iterator>* p, Iterator* ptr, std::size_t* size)
{
...
template<parser_mode M, typename Iterator>
int
fj_parse_number(fj_parser<Iterator>* p, Iterator* ptr, std::size_t* size)
{
    Iterator start = p->js_cur;
...
template<parser_mode M, typename Iterator>
int
fj_parse_value(fj_parser<Iterator>* p, Iterator* ptr, std::size_t* size, e_fj_token_type* toktype, fj_token<Iterator>* parent);

template<parser_mode M, typename Iterator>
int
fj_parse_array(fj_parser<Iterator>* p, fj_token<Iterator>* parent)
{
    int ec = fj_check_and_skip(p, '[');
    if (ec)
        return ec;
    ...
    while (__FLATJSON__CUR_CHAR(p) != ']') {
        ...
        std::size_t size = 0;
        ec = fj_parse_value<M>(p, &(pair->__val), &size, &(pair->__type), startarr);
        if (ec)
            return ec;
        if (M == parser_mode::parse) {
...
template<parser_mode M, typename Iterator>
int
fj_parse_object(fj_parser<Iterator>* p, fj_token<Iterator>* parent)
{
    int ec = fj_check_and_skip(p, '{');
    if (ec)
        return ec;
    ...
    while (__FLATJSON__CUR_CHAR(p) != '}') {
        ...
        std::size_t size = 0;
        ec = fj_parse_value<M>(p, &(pair->__key), &size, &(pair->__type), startobj);
        if (ec)
            return ec;
        if (M == parser_mode::parse) {
        ...
        if (ch == '[' || ch == '{') {
            ...
            ec = fj_parse_value<M>(p, &unused_str, &unused_size, &(pair->__type), startobj);
        } else {
            if (M == parser_mode::parse) {
            ...
            ec = fj_parse_value<M>(p, &(pair->__val), &size, &(pair->__type), startobj);
            __FLATJSON__CHECK_OVERFLOW(size, __FLATJSON__VLEN_TYPE, FJ_EC_VLEN_OVERFLOW);
            if (M == parser_mode::parse)
                pair->__vlen = size;
        }
...
template<typename Iterator>
inline void
fj_init(fj_parser<Iterator>* parser, Iterator beg, Iterator end, fj_token<Iterator>* tokbuf, std::size_t toksize)
{
    // root token
...
template<typename Iterator>
inline parse_result
fj_parse(fj_parser<Iterator>* parser)
{
    parse_result res{};

    std::size_t vlen = 0;
    res.ec = static_cast<e_fj_error_code>(fj_parse_value<parser_mode::parse, Iterator>(
        parser, &(parser->jstok_beg->__val), &vlen, &(parser->jstok_beg->__type), nullptr));
...
template<typename Iterator>
inline fj_parser<Iterator>
fj_make_parser(fj_token<Iterator>* tokbuf, std::size_t toksize, Iterator beg, Iterator end)
{
...
template<typename Iterator>
inline parse_result
fj_num_tokens(Iterator beg, Iterator end)
{
    parse_result res{};
    fj_parser<Iterator> parser{};
    parser.js_cur = beg;
    ...
    res.ec = static_cast<e_fj_error_code>(
        fj_parse_value<parser_mode::count_tokens, Iterator>(&parser, &(parser.jstok_beg->__val), &vlen, &toktype, nullptr));
...
template<typename Iterator>
inline void
fj_dump_tokens(std::FILE* stream, fj_token<Iterator>* toks, std::size_t num)
{
    for (auto* it = toks; it != toks + num; ++it) {
        std::fprintf(stream,
                     "%2d: type=%12s, parent=%p, addr=%p, end=%p, childs=%d, key=%.*s, val=%.*s\n",
                     (int)(it - toks),
...
inline std::size_t
fj_get_tokens(const fj_token<Iterator>* toks, std::size_t num, std::size_t indent, void* userdata, fj_gather_cb_t cb)
{
...
tokens_to_stream_cb_0(void* userdata, const char* ptr, std::size_t len)
{
...
template<typename Iterator>
inline std::size_t
fj_tokens_to_stream(std::FILE* stream, const fj_token<Iterator>* toks, std::size_t num, std::size_t indent = 0)
{
...
static void
tokens_to_stream_cb_1(void* userdata, const char* ptr, std::size_t len)
{
...
template<typename Iterator>
inline std::size_t
fj_tokens_to_stream(std::ostream& stream, const fj_token<Iterator>* toks, std::size_t num, std::size_t indent = 0)
{
...
static void
tokens_to_buf_cb(void* userdata, const char* ptr, std::size_t len)
{
...
template<typename Iterator>
inline std::size_t
fj_tokens_to_buf(const fj_token<Iterator>* toks, std::size_t num, char* buf, std::size_t size, std::size_t indent = 0)
{
...
template<typename Iterator>
inline std::size_t
fj_str_length(const fj_token<Iterator>* toks, std::size_t num, std::size_t indent = 0)
{
...
template<typename Iterator>
inline std::size_t
fj_get_keys(const fj_token<Iterator>* toks, std::size_t num, void* userdata, fj_gather_cb_t cb)
{
    std::size_t cnt{};
...
public:
    ...
    explicit fjson(std::size_t reserved = 0) :
        m_storage{ std::make_shared<storage_type>(reserved) }, m_src_beg{ nullptr }, m_src_end{ nullptr }, m_beg{ nullptr },
        m_end{ nullptr }, m_err{}
    {
    }
    ...
    explicit fjson(const CharT (&str)[L], std::size_t reserved = 0) :
        ...
        m_src_end{ str + L - 1 }, m_beg{ nullptr }, m_end{ nullptr }, m_err{}
    {
        load(str, L - 1);
    }
...
private:
    fjson(storage_ptr storage, element_type* beg, element_type* end) :
        m_storage{ std::move(storage) }, m_src_beg{ nullptr }, m_src_end{ nullptr }, m_beg{ beg }, m_end{ end }, m_err{}
    {
    }
...
public:
    ...
    bool contains(const char (&key)[KL]) const
    {
    ...
    // for objects
    template<std::size_t KL>
    fjson at(const char (&key)[KL]) const
    {
        return at(key, KL - 1);
    }
    ...
    // for objects
    template<std::size_t KL>
    fjson operator[](const char (&key)[KL]) const
    {
        return at(key, KL - 1);
    }
    ...
    bool load(InputIterator beg, InputIterator end)
    {
        ...
        details::fj_init(&parser, beg, end, &(*m_storage->begin()), m_storage->size());
    ...
    std::string dump(std::size_t indent = 0) const
    {
        ...
        assert(strlen == details::fj_tokens_to_buf(m_beg, m_storage->size(), &res[0], res.size(), indent));
    ...
    std::pair<const fj_token<InputIterator>*, std::size_t> data() const { return { m_beg, m_end - m_beg }; }

    std::vector<static_string> get_keys() const
    {
    ...
    std::pair<InputIterator, InputIterator> get_source_data() const { return { m_src_beg, m_src_end }; }
...
private:
    std::pair<element_type*, element_type*> find(const char* key, std::size_t len) const
    {
...
template<typename Iterator>
fjson
parse(Iterator beg, Iterator end)
{
    fjson json{ beg, end };
