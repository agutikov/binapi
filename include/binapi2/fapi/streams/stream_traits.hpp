...
///        target paths, event types, and (post-2026) URL category.
///
/// Each `stream_traits<T>` specialization declares a constexpr
/// `category` matching Binance's `/public` vs `/market` URL split (see
/// `stream_category.hpp`). Single-stream URLs are derived from this
/// category at compile time; combined-stream URLs are picked by the
/// connection at construction time, and the `same_category` concept
/// rejects subscribe-mixing at compile time.

#pragma once

#include <binapi2/fapi/config.hpp>
#include <binapi2/fapi/stream_category.hpp>
...
/// Build a single-stream URL target from the category and topic.
/// `cfg` is unused — the category determines the prefix — but kept in
/// the signature for backward source-compat with callers that pass a
/// config through.
inline ws_target_t
target_from_category(stream_category_e category, std::string topic)
{
    return std::string{ category_single_target(category) } + "/" + std::move(topic);
}
...
template<> struct stream_traits<types::aggregate_trade_subscription> {
    ...
    { return detail::target_from_category(category, topic(cfg, sub)); }
};

template<> struct stream_traits<types::book_ticker_subscription> {
    static constexpr stream_category_e category = stream_category_e::public_;
...
/// All argument types share the same `stream_traits<T>::category`.
/// Used by combined-stream connections to refuse SUBSCRIBE calls that
/// would mix categories — Binance silently drops cross-category frames
/// and the failure mode is otherwise indistinguishable from a dead IP.
template<class First, class... Rest>
concept same_category =
    has_stream_traits<First> && (has_stream_traits<Rest> && ...) &&
    ((stream_traits<First>::category == stream_traits<Rest>::category) && ...);
