...
{
    symbol_t symbol{};                                        ///< Trading pair symbol.
    kline_interval_t interval{ kline_interval_t::m1 }; ///< Candlestick interval (e.g. 1m, 5m, 1h).
};

/// @brief Parameters for subscribing to the continuous contract kline_t stream.
struct continuous_contract_kline_subscription
{
    pair_t pair{};                                                                  ///< Trading pair (e.g. "BTCUSDT").
    contract_type_t contract_type{ contract_type_t::perpetual }; ///< Contract type (perpetual, current quarter, etc.).
    kline_interval_t interval{ kline_interval_t::m1 };           ///< Candlestick interval.
};
