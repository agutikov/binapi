...
#define __BINAPI_CB_ON_ERROR(cb, ec) cb(__FILE__ "(" BOOST_PP_STRINGIZE(__LINE__) ")", ec.value(), ec.message(), nullptr, 0);
...
struct websocket : std::enable_shared_from_this<websocket>
{
    ...
    explicit websocket(boost::asio::io_context& ioctx) :
        m_ioctx{ ioctx }, m_ssl{ boost::asio::ssl::context::sslv23_client }, m_resolver{ m_ioctx }, m_ws{ m_ioctx, m_ssl },
    ...
    void async_start(const std::string& host, const std::string& port, const std::string& target, CB cb, holder_type holder)
    {
        m_host = host;
        m_target = target;
...
private:
    ...
    {
        m_ws.control_callback([this](boost::beast::websocket::frame_type kind, boost::beast::string_view payload) mutable {
            (void)kind;
            (void)payload;
            // std::cout << "control_callback(" << this << "): kind=" << static_cast<int>(kind) << ", payload=" <<
        ...
        m_ws.next_layer().async_handshake(
            ...
            [this, cb = std::move(cb), holder = std::move(holder)](boost::system::error_code ec) mutable {
                if (ec) {
                    if (!m_stop_requested) {
                        __BINAPI_CB_ON_ERROR(cb, ec);
                    }
    ...
    void on_async_ssl_handshake(CB cb, holder_type holder)
    {
        m_ws.async_handshake(
            m_host, m_target, [this, cb = std::move(cb), holder = std::move(holder)](boost::system::error_code ec) mutable {
    ...
    void start_read(boost::system::error_code ec, CB cb, holder_type holder)
    {
        ...
        m_ws.async_read(
            m_buf,
            [this, cb = std::move(cb), holder = std::move(holder)](boost::system::error_code ec, std::size_t rd) mutable {
...
struct websockets::impl
{
    ...
    boost::intrusive::set<
        ...
        boost::intrusive::member_hook<websocket, boost::intrusive::set_member_hook<>, &websocket::m_intrusive_set_hook>>
        m_set;
};
...
websockets::websockets(boost::asio::io_context& ioctx,
                       ...
                       std::size_t stat_interval) :
    pimpl{
        std::make_unique<impl>(ioctx, std::move(host), std::move(port), std::move(msg_cb), std::move(stat_cb), stat_interval)
    }
{
}
...
websockets::handle
websockets::diff_depth(const char* pair, e_freq freq, on_diff_depths_received_cb cb)
{
...
websockets::handle
websockets::klines(const char* pair, const char* interval, on_kline_received_cb cb)
{
...
websockets::handle
websockets::trade(const char* pair, on_trade_received_cb cb)
{
...
websockets::handle
websockets::agg_trade(const char* pair, on_agg_trade_received_cb cb)
{
...
websockets::handle
websockets::mini_ticker(const char* pair, on_mini_ticker_received_cb cb)
{
    return pimpl->start_channel(pair, "miniTicker", std::move(cb));
}

websockets::handle
websockets::mini_tickers(on_mini_tickers_received_cb cb)
{
...
websockets::handle
websockets::market(const char* pair, on_market_received_cb cb)
{
    return pimpl->start_channel(pair, "ticker", std::move(cb));
}

websockets::handle
websockets::markets(on_markets_received_cb cb)
{
...
websockets::handle
websockets::book(const char* pair, on_book_received_cb cb)
{
...
websockets::handle
websockets::userdata(const char* lkey,
...
void
websockets::unsubscribe(const handle& h)
{
...
{
    return pimpl->async_stop_channel(h);
}

void
websockets::unsubscribe_all()
{
...
{
    return pimpl->async_unsubscribe_all();
}
