...
#define __CATCH_BLOCK_WITH_USERCODE(os, exception, ...)                                                                        \
    ...
    {                                                                                                                          \
        __CATCH_BLOCK_WRITES_TO_STDOUT_EXPAND_EXPR(os << __MESSAGE("[" BOOST_PP_STRINGIZE(exception) "]: " << ex.what())       \
                                                                   << std::endl;)                                              \
        {                                                                                                                      \
...
#define __CATCH_BLOCK_CB(unused0, data, elem)                                                                                  \
    BOOST_PP_IF(BOOST_PP_GREATER(BOOST_PP_TUPLE_SIZE(elem), 1), __CATCH_BLOCK_WITH_USERCODE, __CATCH_BLOCK_WITHOUT_USERCODE)(  \
...
std::string
b2a_hex(const std::uint8_t* p, std::size_t n)
{
...
std::string
hmac_sha256(const char* key, std::size_t klen, const char* data, std::size_t dlen)
{
...
// unused for now
bool
verify_signature(const unsigned char* sig, std::size_t slen, const char* data, std::size_t dlen)
{
    bool result = true;
    ...
    if (1 != (EVP_DigestInit_ex(ctx, md, nullptr) && EVP_DigestVerifyInit(ctx, nullptr, md, nullptr, vkey) &&
...
// unused for now
std::string
rsa_sha256(const char* privkeyfile, std::size_t /*pklen*/, const char* data, std::size_t dlen)
{
    ...
    if (1 != (EVP_DigestSignInit(mdctx, nullptr, ::EVP_sha256(), nullptr, pkey) && EVP_DigestSignUpdate(mdctx, data, dlen) &&
...
struct api::impl
{
    impl(boost::asio::io_context& ioctx,
        ...
        m_ioctx{ ioctx }, m_host{ std::move(host) }, m_port{ std::move(port) }, m_pk{ std::move(pk) }, m_sk{ std::move(sk) },
        m_timeout{ timeout }, m_client_api_string{ std::move(client_api_string) }, m_write_in_process{}, m_async_requests{},
        m_ssl_ctx{ boost::asio::ssl::context::sslv23_client }, m_resolver{ m_ioctx }
    {
    }
    ...
    void async_post()
    {
        ...
        m_resolver.async_resolve(m_host,
                                 m_port,
                                 [this, req = std::move(req)](const boost::system::error_code& ec,
    ...
    void on_connect(const boost::system::error_code& ec, request_ptr req, ssl_socket_ptr ssl_socket)
    {
        ...
        sptr->async_handshake(
            ...
            [this, req = std::move(req), ssl_socket = std::move(ssl_socket)](const boost::system::error_code& ec) mutable {
...
api::result<server_time_t>
api::server_time(server_time_cb cb)
{
...
api::result<prices_t::price_t>
api::price(const char* symbol, price_cb cb)
{
...
api::result<avg_price_t>
api::avg_price(const char* symbol, avg_price_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol } };
...
api::result<_24hrs_tickers_t::_24hrs_ticker_t>
api::_24hrs_ticker(const char* symbol, api::_24hrs_ticker_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol } };
...
api::result<_24hrs_tickers_t>
api::_24hrs_tickers(api::_24hrs_tickers_cb cb)
{
...
api::result<exchange_info_t>
api::exchange_info(exchange_info_cb cb)
{
    return pimpl->post(false, "/api/v3/exchangeInfo", boost::beast::http::verb::get, {}, std::move(cb));
}

api::result<exchange_info_t>
api::exchange_info(const char* symbol, exchange_info_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol } };
...
api::result<exchange_info_t>
api::exchange_info(const std::vector<std::string>& symbols, exchange_info_cb cb)
{
    std::string symstr = "[";
...
api::result<depths_t>
api::depths(const char* symbol, std::size_t limit, depths_cb cb)
{
...
api::result<trades_t::trade_t>
api::trade(const char* symbol, trade_cb cb)
{
...
api::result<trades_t>
api::trades(const char* symbol, std::size_t limit, trades_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol }, { "limit", limit } };
...
api::result<agg_trades_t::agg_trade_t>
api::agg_trade(const char* symbol, agg_trade_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol }, { "limit", 1u } };
...
api::result<agg_trades_t>
api::agg_trades(const char* symbol, std::size_t limit, agg_trades_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol }, { "limit", limit } };
...
api::result<klines_t>
api::klines(const char* symbol, const char* interval, std::size_t limit, klines_cb cb)
{
...
api::result<account_info_t>
api::account_info(account_info_cb cb)
{
...
api::result<order_info_t>
api::order_info(const char* symbol, std::size_t orderid, const char* client_orderid, order_info_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol }, { "orderId", orderid }, { "origClientOrderId", client_orderid } };
...
api::result<orders_info_t>
api::open_orders(const char* symbol, open_orders_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol } };
...
api::result<orders_info_t>
api::all_orders(const char* symbol,
                std::size_t orderid,
                ...
                all_orders_cb cb)
{
    const impl::init_list_type map = {
        { "symbol", symbol }, { "orderId", orderid }, { "startTime", start_time }, { "endTime", end_time }, { "limit", limit }
    };
...
api::new_order(const char* symbol,
               const e_side side,
               const e_type type,
               const e_time time,
...
api::result<new_order_resp_type>
api::new_test_order(const char* symbol,
                    const e_side side,
                    const e_type type,
                    const e_time time,
                    const e_trade_resp_type resp,
...
api::result<cancel_order_info_t>
api::cancel_order(const char* symbol,
                  std::size_t order_id,
                  const char* client_order_id,
...
api::result<cancel_all_open_orders_info_t>
api::cancel_all_open_orders(const char* symbol, cancel_all_open_orders_cb cb)
{
    const impl::init_list_type map = { { "symbol", symbol } };
...
api::result<my_trades_info_t>
api::my_trades(const char* symbol,
               std::size_t start_time,
               std::size_t end_time,
               std::size_t from_id,
               std::size_t limit,
...
{
    const impl::init_list_type map = {
        { "symbol", symbol }, { "startTime", start_time }, { "endTime", end_time }, { "fromId", from_id }, { "limit", limit }
    };
...
api::result<start_user_data_stream_t>
api::start_user_data_stream(start_user_data_stream_cb cb)
{
...
api::result<ping_user_data_stream_t>
api::ping_user_data_stream(const char* listen_key, ping_user_data_stream_cb cb)
{
...
api::result<close_user_data_stream_t>
api::close_user_data_stream(const char* listen_key, close_user_data_stream_cb cb)
{
    const impl::init_list_type map = { { "listenKey", listen_key } };
