...
std::uint64_t
mstime_from_str(const std::string& strtime)
{
...
get_symbols_of_nonempty_balances(const binapi::rest::account_info_t& accinfo)
{
...
std::vector<std::string>
get_pairs_for_pairs(const rest::account_info_t& accinfo,
...
void
remove_all_buy_trades(trade_info_container_t& trades)
{
...
std::vector<cycle_pair>
get_full_cycle_trades(binapi::rest::api& api,
                      ...
                      const std::function<void(const rest::order_info_t&)>& tick)
{
    ...
    for (auto& oit : trades) {
        ...
        if (tick) {
            tick(sell_order_info.v);
        }
        if (sell_order_info.v.clientOrderId.empty()) {
            continue;
        }
        if (sell_order_info.v.status == "PARTIALLY_FILLED") {
            continue;
        }
...
pair_trades
calc_pair_trades_info(const std::vector<cycle_pair>& trades)
{
    pair_trades res;
...
void
make_trades_report(std::ostream& os,
                   rest::api& api,
                   ...
                   const std::string& start_time_str)
{
    ...
    const auto mtime_from = start_time > 0 ? start_time : !start_time_str.empty() ? mstime_from_str(start_time_str) : 0;
...
void
make_trades_report_for_last_day(std::ostream& os,
                                rest::api& api,
                                const rest::account_info_t& accinfo,
                                const rest::exchange_info_t& exinfo,
                                const std::vector<std::string>& pairs,
...
get_open_orders(rest::api& api,
                ...
                const std::string& start_time_str)
{
    const auto mstart_time = start_time > 0 ? start_time : !start_time_str.empty() ? mstime_from_str(start_time_str) : 0;
...
void
make_open_orders_resume(std::ostream& os, binapi::rest::orders_info_t& orders, const binapi::rest::exchange_info_t& exinfo)
{
    ...
    for (const auto& pit : orders.orders) {
        ...
        if (buy_count) {
            os << "  B: " << buy_count << " orders, total: " << buy_amount << " " << sym.baseAsset << " = " << buy_for_pair
               << " " << sym.quoteAsset << std::endl;
        }
        if (sell_count) {
            os << "  S: " << sell_count << " orders, total: " << sell_amount << " " << sym.baseAsset << " = " << sell_for_pair
               << " " << sym.quoteAsset << std::endl;
        }
        os << std::endl;
    }
...
void
make_open_orders_detailed(std::ostream& os, binapi::rest::orders_info_t& orders, const binapi::rest::exchange_info_t& exinfo)
{
    (void)exinfo;
    ...
    for (auto& pit : orders.orders) {
        os << pit.first << ":" << std::endl;
        std::sort(pit.second.begin(), pit.second.end(), [](const rest::order_info_t& l, const rest::order_info_t& r) {
...
void
make_open_orders_report(std::ostream& os,
                        rest::api& api,
                        const rest::exchange_info_t& exinfo,
                        const std::vector<std::string>& pairs,
                        const std::function<void(const std::string&)>& tick,
...
void
make_balance_report(std::ostream& os, rest::api& api, const rest::account_info_t& accinfo, const rest::exchange_info_t& exinfo)
{
    (void)api;
    (void)exinfo;
...
void
show_exchanger_price_for_orders(std::ostream& os,
                                rest::api& api,
                                const rest::exchange_info_t& exinfo,
                                const std::vector<std::string>& pairs,
                                const std::function<void(const std::string&)>& tick)
{
    ...
    for (const auto& it : out_items) {
        const auto price_prec =
            binapi::num_fractions_from_double_type(exinfo.get_by_symbol(it.pair).get_filter_price().tickSize);
        os << it.pair << ": " << it.exchanger_price.str(price_prec, iofmt) << ", "
...
void
calc_loss_for_orders(std::ostream& os,
                     rest::api& api,
                     const rest::exchange_info_t& exinfo,
                     const std::vector<std::string>& pairs,
                     const std::function<void(const std::string&)>& tick)
{
    ...
    // sort by loss
    std::sort(out_items.begin(), out_items.end(), [](const out_item& l, const out_item& r) { return l.loss > r.loss; });
    ...
    for (const auto& it : out_items) {
        ...
        const auto price_prec =
            binapi::num_fractions_from_double_type(exinfo.get_by_symbol(it.pair).get_filter_price().tickSize);
        os << it.pair << ": " << it.loss.str(price_prec, iofmt) << " " << sym.quoteAsset << std::endl;
    }
    os << "   TOTAL LOSS: " << total_loss.str(2, iofmt) << std::endl;
}
