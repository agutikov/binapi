...
template <typename T>
struct glz::meta<binapi2::umf::websocket_api::wire_request<T>> {
    using U = binapi2::umf::websocket_api::wire_request<T>;
    static constexpr auto value = object("id", &U::id, "method", &U::method, "params", &U::params);
};

template <>
struct glz::meta<binapi2::umf::websocket_api::session_logon_wire_request> {
    using T = binapi2::umf::websocket_api::session_logon_wire_request;
...
result<types::websocket_api_response<Response>> decode_rpc_response(const std::string &raw) {
...
template <typename Params>
result<std::string> encode_rpc_request(const std::string &id, std::string_view method, const Params &params) {
...
types::websocket_api_signed_request make_signed_request_base(const config &cfg) {
...
result<types::websocket_api_response<Response>> send_rpc(transport::websocket_client &transport, const std::string &id, std::string_view method, const Params &params) {
...
result<types::websocket_api_response<types::session_logon_result>> client::session_logon() {
    query_map auth_query;
...
result<types::websocket_api_response<types::account_information>> client::account_status() {
    return send_rpc<types::account_information>(transport_, next_id(), account_status_method, make_signed_request_base(cfg_));
}

result<types::websocket_api_response<std::vector<types::futures_account_balance>>> client::account_balance() {
    return send_rpc<std::vector<types::futures_account_balance>>(transport_, next_id(), account_balance_method, make_signed_request_base(cfg_));
}

result<types::websocket_api_response<types::order_response>> client::new_order(const types::new_order_request &request) {
...
result<types::websocket_api_response<types::order_response>> client::query_order(const types::query_order_request &request) {
    auto auth = make_signed_request_base(cfg_);
...
result<types::websocket_api_response<types::order_response>> client::cancel_order(const types::cancel_order_request &request) {
    auto auth = make_signed_request_base(cfg_);
...
result<types::websocket_api_response<types::book_ticker>> client::book_ticker(const types::book_ticker_request &request) {
    return send_rpc<types::book_ticker>(transport_, next_id(), ticker_book_method, types::websocket_api_book_ticker_request{request.symbol});
}
