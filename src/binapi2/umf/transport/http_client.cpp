...
result<http_response> http_client::request(
...
void http_client::async_request(
    boost::beast::http::verb method,
    std::string target,
    std::string body,
    ...
    boost::asio::post(io_context_, [this, method, target = std::move(target), body = std::move(body), content_type = std::move(content_type), api_key = std::move(api_key), callback = std::move(callback)]() mutable {
        callback(request(method, target, body, content_type, api_key));
    });
}
